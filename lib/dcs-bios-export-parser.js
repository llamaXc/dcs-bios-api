...
class DcsBiosExportParser extends EventEmitter {
  ...
  processDataChangeOnAddress(addressArray, dataArray){
    const address = addressArray[0]
    const newData = dataArray[0]
    this._data[address] = newData

    this._parseMessage(address, newData)
  ...
  constructor(options = {}) {
    ...
    this.protocolParser = new ProtocolParser( 
      (address, data) => this.processDataChangeOnAddress(address, data),
      () => this._processEmitQueue()
    );

    this._client.on('message', (message) => {
      const d = new DataView(message.buffer);
      for (let i = 0; i < d.byteLength; i++) {
          this.protocolParser.processChar(d.getUint8(i));
      }
    });

    if (this._options.autoStartListening) {
      this.startListening();
    }
  }
   ...
   */
  _parseMessage(address, data) {
    // Oops can't find anything here! Lets safely exit 
    if (this._addressLookup[address] === undefined){
      return
    }

    this._addressLookup[address].forEach((control) => {

      control.outputs.forEach((output) => {

        // Only update the output if the most recent data was sent for the given address
        if(output.address === address){
          var controlValue; 

          if (output.type == 'string') {
            controlValue = this._data.toString('utf8', address, address + output.max_length);
          }else {
            controlValue = (data & output.mask) >> output.shift_by;
          }
